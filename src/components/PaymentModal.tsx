...
const upiProviders = [
  { id: 'gpay', name: 'GPay', color: 'bg-white text-[#17351F] border-[#17351F]' },
  { id: 'phonepe', name: 'PhonePe', color: 'bg-[#5f259f] text-white border-[#5f259f]' },
  { id: 'paytm', name: 'Paytm', color: 'bg-[#002970] text-white border-[#002970]' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#111511]/60 backdrop-blur-sm">
      <motion.div 
        ...
        className="bg-white w-full max-w-sm rounded-sm shadow-xl overflow-hidden relative border border-[#C9D7B5] p-6 sm:p-8 flex flex-col max-h-[90vh] overflow-y-auto scrollbar-hide"
      >
        <button 
          onClick={onCancel} 
        ...
        <div className="bg-[#F5F8EC] border border-[#C9D7B5] rounded-sm p-4 mb-6 flex justify-between items-center shrink-0">
        ...
        <div className="flex gap-1 mb-6 bg-[#F5F8EC] p-1 rounded-sm border border-[#C9D7B5]/50 shrink-0">
              ...
              {paymentMethod === 'upi' && (
                ...
                >
                  <div className="w-full">
                    <p className="text-[10px] uppercase font-bold tracking-widest text-[#17351F]/60 text-center w-full mb-3">
                      Select UPI App
                    </p>
                    <div className="grid grid-cols-2 gap-2 w-full">
                      {upiProviders.map(provider => (
                        <button
                          ...
                          className={cn(
                            "py-2.5 text-[10px] font-bold tracking-widest rounded-sm border transition-all flex items-center justify-center",
                            upiProvider === provider.id
                              ? provider.color + " shadow-sm ring-1 ring-offset-1 ring-current"
                  ...
                  <div className="bg-[#F5F8EC] border border-[#C9D7B5] rounded-sm p-5 flex flex-col items-center justify-center w-full relative overflow-hidden">
                    {/* Decorative background element */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-[#C8E87A]/20 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none"></div>
                    
                    <div className="relative w-40 h-40 bg-white border border-[#C9D7B5] p-2.5 rounded-lg flex items-center justify-center shadow-inner z-10 mb-4 overflow-hidden">
                       {/* TODO: INTEGRATION POINT - Replace 'test@upi' with a dynamically generated merchant UPI ID from your backend payment intent */}
                       <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=upi://pay?pa=test@upi&pn=TakeTheSpot&am=${(amount/100).toFixed(2)}&cu=USD`} alt={`${upiProvider} QR Code`} className="w-full h-full mix-blend-multiply" />
                       
                       {/* Animated scanning line for a secure/tech feel */}
                       <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden rounded-lg">
                         <motion.div 
                           animate={{ y: ["0%", "100%", "0%"] }}
                           transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
                           className="w-full h-[2px] bg-[#C8E87A] shadow-[0_0_12px_3px_rgba(200,232,122,0.8)]"
                         />
                       </div>
                    </div>
                    
                    <div className="flex flex-col items-center gap-2 z-10">
                      <span className="text-[11px] uppercase tracking-widest font-black text-[#17351F]">
                        Scan with {upiProviders.find(p => p.id === upiProvider)?.name}
                      </span>
                      
                      <div className="flex items-center gap-1.5 text-[#17351F]/80 bg-white/60 px-2.5 py-1 rounded-sm border border-[#C9D7B5]/60 shadow-sm backdrop-blur-sm">
